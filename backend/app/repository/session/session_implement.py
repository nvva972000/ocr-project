...
class SessionImplement(SessionInterface):
    ...
    def delete_session_by_refresh_token(self, refresh_token: str) -> bool:
        ...
        try:
            session = (
                session_db.query(SessionEntity)
                .filter(SessionEntity.refresh_token == refresh_token)
                .first()
            )
            if session:
            ...
            session_db.rollback()
            logging.error(f"Error when delete session by refresh token: {e}")
        finally:
            session_db.close()
        return False
    ...
    def delete_sessions_by_user_id(self, user_id: str) -> bool:
        ...
        try:
            ...
            session_db.rollback()
            logging.error(f"Error when delete sessions by user id: {e}")
        finally:
            session_db.close()
        return False
    ...
    def get_active_sessions(self) -> list:
        ...
        session_db = Session()
        try:
            from app.models.session import STATUS_SESSION_ACTIVE
            sessions = (
                session_db.query(SessionEntity)
                .filter(SessionEntity.status == STATUS_SESSION_ACTIVE)
                .all()
            )
            return sessions if sessions else []
        except Exception as e:
            logging.error(f"Error when get active sessions: {e}")
        finally:
            session_db.close()
        return []
