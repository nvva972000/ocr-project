...
const Org: React.FC = () => {
  ...
  const handleSyncOrgs = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      message.success("Organizations synced successfully");
    }, 1000);
  };

  const handleEdit = useCallback((orgId: string) => {
    message.info(`Edit organization: ${orgId}`);
  }, []);

  const handleDelete = useCallback((payload: { id: string; name: string }) => {
    message.warning(`Delete organization: ${payload.name}`);
  }, []);

  const handleToggleActive = useCallback((orgId: string) => {
    message.info(`Toggle active status for organization: ${orgId}`);
  }, []);
